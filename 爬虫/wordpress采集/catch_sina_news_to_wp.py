...
def geturls(url):
    ...
    targeturl=tree.xpath("//div[@class='contList']/ul/li/a/@href")
    print(targeturl)
    re_anly(targeturl)
...
def re_anly(targeturl):
    for i in range(0,3):
        ...
        #利用正则获取文章的标题
        d = re.compile(r'<h1 id="main_title">(.*?)</h1>',re.S)
        w = d.findall(code2.decode('UTF-8'))
        #print('CODE2: %s' %code2.decode('GBK'))
        title = w[0]
        print('title: %s %s' %(title,type(title)))
        #利用正则获取文章的内容（含内容中的图片链接）
        c = re.compile(r'<div class="content" id="artibody" data-sudaclick="blk_content">(.*)<!-- tech_web_article_v2015_block_bottom -->',re.S)
...
def sent_to_wp(filename,fullname,title,content):
    ...
    wp = Client('http://www.abc.com/xmlrpc.php', 'www', 'abc22')
    post = WordPressPost()
    post.title = title
...
if __name__=='__main__':
    ...
    geturls('http://roll.tech.sina.com.cn/internet_worldlist/index.shtml')
    # for i in range(2,6):
